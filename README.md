...
수요 예측/운영 측면에서는 폭염·한파를 핵심 변수로 두기보다, 라인업·요일·시즌성을 중심으로 모델을 구성하고 날씨 변수는 보조적으로 활용하는 전략이 합리적입니다.

Project Overview

본 프로젝트는 2023~2025년 서울시 기온(시간별/일별) 및 폭염·한파 특보 데이터를 예술의전당 공연·전시 입장객 데이터와 결합하여, 극한 기상이 관람객 수 변화에 미치는 영향을 분석했습니다. 향후에는 체감온도·강수·미세먼지 등 상세 기상 지표와 예매/취소 데이터를 추가로 결합하고, 타 문화시설로 분석 범위를 확장하여 일반화 가능성을 높일 예정입니다.
